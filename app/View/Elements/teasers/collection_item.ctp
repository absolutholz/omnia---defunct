<?php 
	...
	$item_status = '';
	if ($collection_item['CollectionItemStatus']['id'] != 1) { 
		$item_status = ' s-' . $collection_item['CollectionItemStatus']['name']; 
	}
	
	$completion_status = '';
	if (isset($completions[$item_id])) { 
		$completion_status = ' s-' . $completions[$item_id]['CompletionStatus']['name'];
	}
